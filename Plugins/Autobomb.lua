...
my_own_section:AddParagraph("Fake Bomb Jump", "Place fake bomb and jump automatically with 20s cooldown. Drag to move the button.")
...
my_own_section:AddToggle("Enable Fake Bomb Jump", function(bool)
    ...
    if bool then
        CreateGUI()
        SetupDragSystem()
        shared.Notify("Fake Bomb Jump enabled! Look for the blue circle button.", 3)
    else
        if ScreenGui then
            ScreenGui:Destroy()
            ScreenGui = nil
